...
descobrir a substring comum mais longa num conjunto de sequências e calcular o máximo de overlap de um conjunto de sequências.
"""
...
class SuffixTree:
        ...
        while pos < len(p): # enquanto a primeira posição do padrão for menor que o tamanho do padrão,
            if p[pos] not in self.nodes[node][1].keys(): # se a posição inicial do padrão não estiver presente nas keys do dicionário de nodes (se ainda não existir um arco),
                if pos == len(p) - 1: # se a posição inicial for a última do padrão,
                    self.add_node(node, p[pos], sufnum) # adiciona um node com o sufixo
                else: # caso contrário,
                    self.add_node(node, p[pos]) # não adiciona um sufixo
    ...
    def suffix_tree_from_seq(self, text: str) -> None:
        """
        Método que cria a árvore de sufixos, adicionando um símbolo ("$") no fim da sequência e chama o método anterior para cada sufixo da sequência -> nenhum sufixo será o prefixo de outro sufixo.
    ...
    def find_pattern(self, pattern: str):
        ...
        return self.get_leaves_below(node) # no fim do ciclo for, retorna o conjunto de folhas abaixo de um dado nó usando o método auxiliar get_leaves_below implementado recursivamente
